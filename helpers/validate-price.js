...
function validatePrice(data) {
   ...
   const stockQuantity = data.stockQuantity;
    ...
    if (isNaN(priceNumber) || priceNumber < 1) {
      return {
        success: false,
        msg: `El precio tiene que ser un número mayor a 1. "${price}" no es un número válido.`,
      };
    }
    if (isNaN(stockQuantityNumber) || stockQuantityNumber < 1) {
      return {
        success: false,
        msg: `La cantidad de stock tiene que ser un número mayor a 1. "${stockQuantity}" no es un número válido.`,
      };
    }
